...
#Splash screen
black_bg= Entity(model='quad', texture='brick', scale=10, parent = camera.ui,color = color.black)
icon = Entity(model='quad', texture='assets/tornado.png', scale=0.5, parent = camera.ui,alpha = 0)
icon.fade_in(duration=3)
menu = Menu()
menu.disable()
s = Sequence(Func(icon.fade_out),Func(black_bg.fade_out), Func(setattr, menu,'enabled',True),duration=4)
s.start()
