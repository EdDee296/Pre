...
class Player(Entity):
    def __init__(self, **kwargs):
        ...
        self.thickness_slider = Slider(
            0.001, 0.1, default=0.0021000028848648074, on_value_changed=self.thick_slider, enabled=False)
        self.length_slider = Slider(
            0.001, 0.1, default=0.006454188346862793, on_value_changed=self.length_sli, enabled=False)
        self.gap_slider = Slider(
            0.001, 0.1, default=0.006270825766026974, on_value_changed=self.gap, enabled=False)
...
class Menu(Entity):
    ...
    def start_game(self):
        ...
        # Game initialization
        ground = Entity(model='plane', scale=(100, 1, 100), color=color.yellow.tint(
            -.2), texture='white_cube', texture_scale=(100, 100), collider='box')
        player = Player(position=Vec3(-4.37877, 0.00100005,
                        3.96363), rotation=Vec3(0, 90, 0))
    ...
    def settings(self):
        ...
        back_button = Button(text='Back', scale=(
            0.2, 0.1), position=(0.2, -0.3))
        entity = [player.gap_slider, player.thickness_slider, player.length_slider,
                  thickness_text, length_text, gap_text, save_button, back_button]
